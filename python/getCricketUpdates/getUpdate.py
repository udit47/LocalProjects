...
class GetCricket:
    ...
    def make_batsman(self, bat, run, ball, s=''):
        batsman_score = self.player.get(bat) + ':' + run + '(' + ball + ')'        if s == '1':
            batsman_score += '*'        return batsman_score + '\n'    def make_batsman_board(self):
        score_board = ''        for bat in self.score.get('batsman',[]):
        ...
        score_board = ''        for bowl in self.score.get('bowler',[]):
    ...
    def in_command(self):
        ...
        return "\"" + self.head + "\n\" \"" + self.body + "\""    def make_notification_command(self):
        event = False        board = ['notify-send', ' -i'] #specific for ubuntu, Customize here        if self.event in self.notify_for:
            event = True            board.append(str(self.icon.get(self.event, self.over_break)))
    ...
    def start_notifications(self):
        old_cmd = ''        count=0        while self.state != 'complete':
            self.data = requests.get(self.url).json()
            self.update_changes()
...
def control():
    ...
    while True:
        ...
        if inp in title_dict.keys() and inp.isdigit():
            break        print('No in choice')
    feature = input('All notifications (y/n)?')
    feature = True if feature.lower()=='y' else False    print('configuring...')
